"""Phase 2B security tests -- app.security.service_jwt (Team 4B).

Mirrors tests/test_service_jwt.py from Team 4A's repository (the two
services use an identical, standalone verifier module/class -- no shared
package exists between the two separately-deployable repositories, so the
module is duplicated, not imported cross-repo). Both `verifier_4a` and
`verifier_4b` fixtures are exercised here since the underlying class and
its cross-service audience/scope isolation guarantees are the same
logic regardless of which repository is running it -- this is the
authoritative test suite for Team 4B's own copy of that logic.
